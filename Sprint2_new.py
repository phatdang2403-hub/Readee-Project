...
# ==========================================================
# (Bỏ qua các biểu đồ để chạy nhanh hơn)
print("\n--- 1: EDA (Đã rút gọn) ---")
...
def clean_text(text):
    ...
    # 1. Xóa tiêu đề/chân trang Gutenberg (Mạnh tay hơn)
    text = re.sub(r'(\*\*\*).*?(\*\*\*)', ' ', text)
...
# =============================================================================
print("\n--- 3: SO SÁNH CÁC MODEL BASELINE (Đã rút gọn) ---")
labels_order = sorted(y.unique())

# --- 3.b. Logistic Regression (Baseline Chính) ---
print("\n--- 3.b. Baseline model 2: Logistic Regression ---")
model_lr = LogisticRegression(max_iter=2000, random_state=RANDOM_STATE, class_weight='balanced') # Tăng max_iter
